...
import {VoucherV2} from "./beacon/VoucherV2.sol";
    ...
    NonTransferableERC20Upgradeable
{
    ...
    function createVoucher(
        ...
        VoucherV2(voucherAddress).initialize(owner, address(this), expiration, voucherType);
    ...
    function topUpVoucher(address voucher, uint256 value) external onlyRole(MINTER_ROLE) {
        ...
        uint256 expiration = block.timestamp +
            $._voucherTypes[VoucherV2(voucher).getType()].duration;
        VoucherV2(voucher).setExpiration(expiration);
    ...
    function drainVoucher(address voucher) external {
        ...
        VoucherV2(voucher).drain(amount);
    }
